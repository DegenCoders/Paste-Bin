...
async def save(request: Request, value: str= Form(...)):
    ...
    saveResponse = session.execute("SELECT content, title FROM notes WHERE category = 'luridarcy' ALLOW FILTERING")
    return templates.TemplateResponse("saved.html", {"request": request, "value" : saveResponse.current_rows})
